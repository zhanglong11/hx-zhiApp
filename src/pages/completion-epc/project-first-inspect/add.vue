<template>
  <view class="main">
    <div class="main-body">
      <!--驳回修改  start-->
      <block v-if="baseInfo.status">
        <uni-card title="审核结果" style="margin-bottom: 15px">
          ...
          <uni-form-item label="审核人">{{ baseInfo.auditPerson }}</uni-form-item>

          <uni-form-item label="审核时间" :border="false">
      ...
      <uni-form-custom ref="form" :model="form" :rules="rules">
        <uni-card isForm>
          ...
          <uni-form-item label="参加单位" prop="participant">
            <textarea v-model.trim="form.participant" auto-height placeholder="请输入参加单位" />
          </uni-form-item>
