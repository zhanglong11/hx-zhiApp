  ...
  <view class="main">
          ...
          <uni-card v-for="item in list" :key="item.id" @click.native="handleItemClick(item)">
            <view class="card-body">
              ...
              >
                ...
                <view>计划验收日期：{{ item.planAcceptanceTime | ymd }}</view>
    ...
    <uni-drawer-custom ref="filterRef" @submit="handleDrawerFormChange" @reset="handleDrawerReset">
      <div>
        ...
        <h6 class="label">整改状态</h6>
        <uni-checkbox-button-group v-model="drawerForm.rectifyStatus" hasAll :options="rectifyStatus" />
...
export default {
  ...
  methods: {
    async refresh() {
      ...
      let res = await Api.getList({ ...data, type: 1 })
    ...
    handleAddInspect() {
      this.$utils.toUrl('/pages/completion-epc/project-first-inspect/add')
    },
    handleItemClick(row) {
      this.$utils.toUrl(
        `/pages/completion-epc/project-first-inspect/detail?id=${row.id}&status=${row.acceptanceStatus}`
      )
    },
