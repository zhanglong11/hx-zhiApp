<template>
  <view class="main">
    ...
    <!--草稿状态-->
    <uni-form-custom
      v-if="info.acceptanceStatus === 0 && info.principalId === userId"
      ref="draftForm"
      :model="draftForm"
      :rules="rules"
    >
      <uni-card isForm>
        <uni-form-item label="验收名称" prop="name">
          <textarea v-model.trim="draftForm.name" auto-height placeholder="请输入验收名称" />
        </uni-form-item>
        <uni-form-item label="验收范围" prop="acceptanceScope">
          <textarea v-model.trim="draftForm.acceptanceScope" auto-height placeholder="请输入验收范围" />
        </uni-form-item>
        <uni-form-item label="计划验收日期" prop="planAcceptanceTime">
          <uni-date-picker v-model="draftForm.planAcceptanceTime" fields="day" mode="date"></uni-date-picker>
        </uni-form-item>
        <uni-form-item label="组织单位" prop="organizer">
          <textarea v-model.trim="draftForm.organizer" auto-height placeholder="请输入组织单位" />
        </uni-form-item>
        <uni-form-item label="参加单位" prop="participant">
          <textarea v-model.trim="draftForm.participant" auto-height placeholder="请输入参加单位" />
        </uni-form-item>
        <uni-form-item label="负责人" isLink prop="principalId">
          <uni-select-user v-model="draftForm.principalId" :name.sync="info.principalName" placeholder="请选择" />
        </uni-form-item>
      </uni-card>
      <!-- 备注和附件信息开始 -->
      <uni-card isForm>
        <uni-form-item label="备注" align="left" class="block-item" prop="remark">
          <textarea v-model.trim="draftForm.remark" :maxlength="-1" placeholder="请输入备注信息" auto-height />
        </uni-form-item>
        <uni-form-item class="file-item" label="竣工验收附件" prop="acceptanceFileId">
          <uni-attachment v-model="draftForm.acceptanceFileId" edit></uni-attachment>
        </uni-form-item>
      </uni-card>
      <!-- 备注和附件信息结束 -->
      <view class="btn-wrapper">
        <button class="button" :disabled="isLoading" type="primary" @click="handleOverInspectSubmit(true)">提交</button>
      </view>
    </uni-form-custom>
    <uni-card v-else isForm>
    ...
    <!--有审核流程开始-->
    <block v-if="info.acceptanceStatus !== 0">
    ...
    <!--流程顺序结束-->
    <!--整改通知详情-->
    <block v-if="info.rectifyStatus === 3">
    ...
    <!-- 草稿提交-->
    <!-- <view v-if="info.acceptanceStatus === 0 && info.principalId === userId" class="btn-wrapper">
...
export default {
  ...
  methods: {
    ...
    //草稿提交
    async handleOverInspectSubmit(isSubmit = true) {
      await this.$refs.draftForm.validate()
      let params = {
        ...this.draftForm,
        optType: isSubmit ? 2 : 1,
        type: 2,
        planAcceptanceTime: moment(this.info.planAcceptanceTime).format('YYYY-MM-DD 00:00:00')
      }
    ...
    //跳转到编辑页面
    handleEdit() {
      this.$utils.toUrl(`/pages/completion-epc/project-over-inspect/add?id=${this.id}`)
    },
    async handleDelete() {
