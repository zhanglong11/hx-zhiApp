    ...
    <view class="swiperContainer">
      ...
      <view class="swiperWrapper">
        <swiper class="swiper" :current="current" @change="handleChange">
          <swiper-item v-for="(item, index) of parentTaskList" :key="index">
        ...
        <uni-card v-for="item in list" :key="item.id" @click.native="handleItemClick(item)">
          <view class="card-body">
            ...
            >
              ...
              <view>工期：{{ moment(item.planEndTime).diff(moment(item.planStartTime), 'days') + 1 }}天</view>
            </view>
...
export default {
  ...
  methods: {
    ...
    async getParentTaskList() {
      let res = await Api.getParentTaskList({ projectId: this.projectId, type: 1 })
    ...
    handleAddParentTask() {
      this.showAdd = false
      this.$utils.toUrl('/pages/early-stage-manage-epc/early-stage-data-prepare/addParent')
    },
    //新建资料准备
    handleAddChildTask() {
      this.showAdd = false
      ...
      uni.showActionSheet({
        ...
        success: function(res) {
          ...
          that.$utils.toUrl(`/pages/early-stage-manage-epc/early-stage-data-prepare/addChild?pid=${parentId}`)
        },
        fail: function(res) {
    ...
    listFilter() {
      //过滤条件
      //this.list = this.copyList.filter(item => item.name.includes(this.filterForm.keyword))
    ...
    handleParentTaskClick(item) {
      this.$utils.toUrl(`/pages/early-stage-manage-epc/early-stage-data-prepare/parentDetail?id=${item.id}`)
    },
    handleItemClick(row) {
      ...
      if (row.userType.includes(1)) {
        if (row.auditStatus === 0 || row.auditStatus === 3) {
          this.$utils.toUrl(
            `/pages/early-stage-manage-epc/early-stage-data-prepare/progressReport?id=${row.id}&pid=${parentId}&userType=1`
          )
          return
        }
      }
      ...
      if (row.userType.includes(2)) {
        if (row.taskStatus === 0) {
          this.$utils.toUrl(
            `/pages/early-stage-manage-epc/early-stage-data-prepare/childDetail?id=${row.id}&userType=2`
          )
          return
        }
        //待审核
        if ([1, 2, 3].includes(row.auditStatus)) {
          this.$utils.toUrl(
            `/pages/early-stage-manage-epc/early-stage-data-prepare/childAudit?id=${row.id}&pid=${parentId}`
          )
          return
        }
      }
      this.$utils.toUrl(`/pages/early-stage-manage-epc/early-stage-data-prepare/childDetail?id=${row.id}`)
    },
