...
export default {
  ...
  methods: {
    ...
    async handleParentSubmit(isSubmit) {
      ...
      let res
      if (this.id) {
        isSubmit ? await Api.submitParentTask(params) : await Api.updateParentTask(params)
      } else {
        res = isSubmit ? await Api.submitParentTask(params) : await Api.addParentTask(params)
      }
      ...
      setTimeout(() => {
        ...
        /* if (this.id) {
          ...
          this.$utils.toUrl(`/pages/early-stage-manage-epc/early-stage-data-prepare/parentDetail?id=${res.data}`)
        }*/
      }, 1500)
    }
  }
