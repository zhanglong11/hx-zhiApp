  ...
  <view class="main">
    <div>
      <uni-form-custom ref="form" class="main-body" :model="form">
        ...
        <view v-if="form.paramDTOList.length">
        ...
        <view v-if="userType.includes('2') && form.taskStatus === 0" class="btn-wrapper">
...
export default {
  ...
  onNavigationBarButtonTap(obj) {
    this.$utils.toUrl(
      `/pages/early-stage-manage-epc/early-stage-data-prepare/addChild?id=${this.id}&pid=${this.form.pid}`
    )
  },
  methods: {
    async getDetail() {
      let res = await Api.getChildTaskDetail(this.id)
      this.form = { ...res.data, projectId: this.projectId, paramDTOList: res.data.paramDTOList || [] }
      // #ifdef  APP-PLUS
      if (this.auditStatus !== 0 || this.type !== 1) {
