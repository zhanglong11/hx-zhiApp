        ...
        <block>
          <uni-card v-for="item in list" :key="item.id" @click.native="handleItemClick(item)">
            ...
            <view v-if="[0, 1].includes(item.auditStatus)" class="card-footer  border-top">
              ...
              <text class="cardTimeValue">执行时间： {{ item.confirmTime || '-' }}</text>
            </view>
            <view v-if="[2, 3].includes(item.auditStatus)" class="card-footer  border-top">
              <text>审核人：{{ item.executorName }}</text>
              <text class="cardTimeValue">审核时间： {{ item.confirmTime || '-' }}</text>
            </view>
          </uni-card>
        </block>
...
export default {
  ...
  methods: {
    ...
    refresh: _.debounce(async function() {
      ...
      let res = await Api.getParentTaskList({ ...data, projectId: this.projectId, archiveFlag: 1, type: 1 })
    ...
    handleItemClick(row) {
      if (row.userType.includes(2)) {
        if (row.auditStatus !== 0) {
          this.$utils.toUrl(`/pages/early-stage-manage-epc/early-stage-file-archive/auditDetail?id=${row.id}`)
          return
        }
      }
      this.$utils.toUrl(
        `/pages/early-stage-manage-epc/early-stage-file-archive/detail?id=${row.id}&userType=${row.userType}`
      )
    },
