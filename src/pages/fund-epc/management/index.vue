<template>
  <div class="main">
    ...
    <uni-add v-if="$hasPower('FundExpensesManagementAddEpc')" @click="handlerAdd"></uni-add>
...
export default {
  ...
  methods: {
    ...
    handlerAdd() {
      this.$utils.toUrl(`/pages/fund-epc/management/add`)
    },
    //searchBar的搜索事件
    ...
    remove() {},
    toDetail(row) {
      this.$utils.toUrl(`/pages/fund-epc/management/detail?id=${row.id}`)
    }
  }
}
</script>
