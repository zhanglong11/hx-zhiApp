  ...
  <div class="main">
    <uni-form-custom ref="form" align="left" hideRequiredAsterisk disabled :model="form" :rules="rules">
      ...
      <div class="income-detail-header">
        <h2>收入计划明细</h2>
        <button
          v-if="$hasPower('FundIncomePlanPaymentEpc')"
          type="primary"
          @click="$utils.toUrl(`/pages/fund-epc/income/actual/index?year=${form.year}&month=${form.month}`)"
        >
          实际收入明细
        </button>
      </div>
      ...
      <view v-if="$hasPower('FundIncomePlanDeleteEpc')" class="btn-wrapper">
...
export default {
  ...
  onShow() {
    ...
    if (!this.$hasPower('FundIncomePlanEditEpc')) {
      this.$utils.hideNavButton.call(this)
    }
  },
  onNavigationBarButtonTap({ key }) {
    if (key === 'edit') {
      this.$utils.toUrl('/pages/fund-epc/income/add?id=' + this.id)
    }
  },
  mounted() {},
  methods: {
