  ...
  <div class="main">
    ...
    <scroll-view scroll-y class="scroll-roll list" @refresherrefresh="init" @scrolltolower="loadMore">
      <div v-for="item in list" :key="item.id" class="card item" @click="toDetail(item)">
        <section>
          <h2>{{ item.planName }}</h2>
          <p>预计收入金额(元)：{{ item.totalAmount }}</p>
          <p>实际收入金额（元）：{{ item.actualAmount || 0 }}</p>
        </section>
        <footer>
    ...
    <uni-add v-if="$hasPower('FundIncomePlanAddEpc')" @click="handlerAdd"></uni-add>
...
export default {
  ...
  methods: {
    ...
    handlerAdd() {
      this.$utils.toUrl(`/pages/fund-epc/income/add`)
    },
    //searchBar的搜索事件
    ...
    remove() {},
    toDetail(row) {
      this.$utils.toUrl(`/pages/fund-epc/income/detail?id=${row.id}`)
    }
  }
}
</script>
