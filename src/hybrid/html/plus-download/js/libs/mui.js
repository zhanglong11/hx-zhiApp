...
var mui = (function(document, undefined) {
  ...
  var $ = function(selector, context) {
    ...
    if (typeof selector === 'object') return wrap([selector], null)
  ...
  $.map = function(elements, callback) {
    ...
    if (typeof elements.length === 'number') {
      //TODO 此处逻辑不严谨，可能会有Object:{a:'b',length:1}的情况未处理
  ...
  $.openWindow = function(url, id, options) {
    ...
    if ($.webviews[id]) {
      ...
      webviewCache.afterShowMethodName &&
        webview.evalJS(webviewCache.afterShowMethodName + "('" + JSON.stringify(params) + "')")
      return webview
    } else {
      //新窗口
        ...
        webview.addEventListener(
          ...
          function() {
            ...
            options.afterShowMethodName &&
              webview.evalJS(options.afterShowMethodName + "('" + JSON.stringify(params) + "')")
          },
          false
        )
      }
        ...
        if (!webview) {
          ...
          if (options.subpages) {
            $.each(options.subpages, function(index, subpage) {
              ...
              var subWebview = plus.webview.create(
                subpage.url,
                subpage.id || subpage.url,
...
;(function($, window) {
  ...
  $.addBack({
    ...
    handle: function() {
      ...
      if (parent) {
        // 增加uni-app返回功能
        parent.evalJS('uni.navigateBack({ delta: 1 });')
      ...
      if (this.parallaxElement && this.options.scrollY) {
        ...
        if (scale > 1) {
          ...
          this.parallaxStyle['webkitTransform'] =
            this._getTranslateStr(0, -parallaxY) + ' scale(' + scale + ',' + scale + ')'
        } else {
    ...
    $.extend(
      {
        ...
        _drag: function(e) {
          ...
          if (this.pulldown) {
            this._setCaption(
              this.y > this.options.down.height ? this.options.down.contentover : this.options.down.contentdown
            )
          }
        },
...
;(function($, document) {
    ...
    $.extend(
      {
        ...
        _pulldownLoading: function() {
          ...
          $.plusReady(function() {
            plus.webview
              .getWebviewById(self.options.webviewId)
              .evalJS('mui&&mui.options.pullRefresh.down&&mui.options.pullRefresh.down.callback()')
          })
        },
        endPulldownToRefresh: function() {
          //该方法是子页面调用的
          var webview = plus.webview.currentWebview()
          webview.parent().evalJS(
            "mui&&mui(document.querySelector('.mui-content')).pullRefresh('" +
              JSON.stringify({
  ...
  $.fn.pullRefresh = function(options) {
    ...
    !options.webviewId &&
      (options.webviewId = plus.webview.currentWebview().id || plus.webview.currentWebview().getURL())
    var pullRefreshApi = null
      ...
      if (!this.slideIn) {
        ...
        if (direction === 'left') {
          shown =
            ...
            this.wrapper.querySelector('.' + CLASS_OFF_CANVAS_LEFT + '.' + CLASS_ACTIVE)
        } else if (direction === 'right') {
          shown =
            this.classList.contains(CLASS_ACTIVE) &&
            this.wrapper.querySelector('.' + CLASS_OFF_CANVAS_RIGHT + '.' + CLASS_ACTIVE)
        } else {
          shown =
            this.classList.contains(CLASS_ACTIVE) &&
            (this.wrapper.querySelector('.' + CLASS_OFF_CANVAS_LEFT + '.' + CLASS_ACTIVE) ||
...
;(function($, window, document, name, undefined) {
  ...
  var handle = function(event, target) {
    if (
      target.classList &&
      (target.classList.contains(CLASS_CONTROL_ITEM) || target.classList.contains(CLASS_TAB_ITEM))
    ) {
